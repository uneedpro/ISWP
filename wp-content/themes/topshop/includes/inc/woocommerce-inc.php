    ...
    ob_start(); ?>
        <a class="header-cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('Открыть корзину', 'topshop'); ?>">
            <span class="header-cart-amount">
                <?php echo sprintf(_n('%d товар', '%d товара', $woocommerce->cart->cart_contents_count, 'topshop'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?>
            </span>
